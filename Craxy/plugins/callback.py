...
from Craxy import YouTube, app
from Craxy.core.call import Crax
from Craxy.misc import SUDOERS, db
from Craxy.utils.database import (is_active_chat,
                                       is_music_playing, set_loop,
                                       is_nonadmin_chat, music_off,
                                       music_on)
from Craxy.utils.decorators.language import languageCB
from Craxy.utils.formatters import seconds_to_min
from Craxy.utils.inline.play import (stream_markup,
                                          telegram_markup)
from Craxy.utils.stream.autoclear import auto_clean
from Craxy.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Crax.pause_stream(chat_id)
        ...
        await Crax.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Crax.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Crax.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Crax.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
