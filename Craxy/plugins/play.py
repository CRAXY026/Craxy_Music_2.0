...
from Craxy import (Apple, Resso, SoundCloud, Spotify, Telegram,
                        YouTube, app)
from Craxy.core.call import Crax
from Craxy.utils import seconds_to_min, time_to_seconds
from Craxy.utils.channelplay import get_channeplayCB
from Craxy.utils.database import is_video_allowed
from Craxy.utils.decorators.language import languageCB
from Craxy.utils.decorators.play import PlayWrapper
from Craxy.utils.formatters import formats
from Craxy.utils.inline.play import (livestream_markup,
                                          playlist_markup,
                                          slider_markup, track_markup)
from Craxy.utils.inline.playlist import botplaylist_markup
from Craxy.utils.logger import play_logs
from Craxy.utils.stream.stream import stream
    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            ...
            if (
                ...
                return await mystic.edit_text(
                    "ᴛʜɪs ʙᴏᴛ ᴄᴀɴ'ᴛ ᴩʟᴀʏ sᴩᴏᴛɪғʏ ᴛʀᴀᴄᴋs ᴀɴᴅ ᴩʟᴀʏʟɪsᴛs, ᴩʟᴇᴀsᴇ ᴅᴏsᴛ ᴄᴏɴᴛᴀᴄᴛ ᴍʏ ᴏᴡɴᴇʀ ᴀɴᴅ ᴀsᴋ ʜɪᴍ ᴛᴏ ᴀᴅᴅ sᴩᴏᴛɪғʏ ᴩʟᴀʏᴇʀ."
                )
            if "track" in url:
                try:
...
@app.on_callback_query(
    filters.regex("CraxyAdmin") & ~BANNED_USERS
)
async def Craxymous_check(client, CallbackQuery):
    try:
        await CallbackQuery.answer(
            "🙂 ᴅᴏsᴛ ʏᴏᴜ'ʀᴇ ᴀɴ ᴠɪʙᴇ ʜᴏᴜsᴇ ᴀᴅᴍɪɴ\n\nʀᴇᴠᴇʀᴛ ʙᴀᴄᴋ ᴛᴏ ᴜsᴇʀ ᴀᴄᴄᴏᴜɴᴛ ғᴏʀ ᴜsɪɴɢ ᴍᴇ.",
            show_alert=True,
        )
    except:
        return
...
@app.on_callback_query(
    filters.regex("CraxPlaylists") & ~BANNED_USERS
)
@languageCB
